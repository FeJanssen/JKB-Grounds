    ...
    try:
        ...
        if response.data:
            ...
            for row in response.data:
                verein = Verein(
                    verein_id=str(row.get('verein_id', '')),
        ...
        logging.error(f"❌ Fehler beim Laden der Vereine: {str(e)}")
...
async def get_verein_by_id(verein_id: str):
    """Lädt einen spezifischen Verein anhand der ID"""
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            logging.info(f"✅ Verein gefunden: {verein.vereinsname}")
            return verein
        else:
            logging.warning(f"❌ Verein mit ID {verein_id} nicht gefunden")
            raise HTTPException(status_code=404, detail=f"Verein mit ID {verein_id} nicht gefunden")
            
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"❌ Fehler beim Laden des Vereins: {str(e)}")
