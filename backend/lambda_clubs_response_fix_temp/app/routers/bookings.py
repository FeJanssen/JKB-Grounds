...
@router.get("/list")
async def get_bookings_list(club_id: str = "fab28464-e42a-4e6e-b314-37eea1e589e6", date: str = None):
    """Bookings list endpoint for frontend compatibility - maps club_id to verein_id"""
    try:
        if not date:
            from datetime import datetime
            date = datetime.now().strftime("%Y-%m-%d")
        
        print(f"📅 FRONTEND COMPAT: Lade Buchungen für Datum: {date}, Club: {club_id}")
        
        bookings = await booking_service.get_bookings_by_date(date, club_id)
        ...
        print(f"❌ FRONTEND COMPAT ERROR: Buchungsabfrage fehlgeschlagen: {e}")
        raise HTTPException(status_code=500, detail="Buchungen konnten nicht geladen werden")
